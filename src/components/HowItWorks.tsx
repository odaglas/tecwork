...
export const HowItWorks = () => {
  return <section id="como-funciona" className="py-20 bg-background scroll-mt-16">
      <div className="container px-4">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold mb-4 text-foreground">
            ¿Cómo Funciona?
          </h2>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">Cuatro simples pasos para encontrar al técnico perfecto</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12">
          {steps.map((step, index) => <div key={step.number} className="relative">
              ...
              {index < steps.length - 1 && <div className="hidden md:block absolute top-10 left-[60%] w-[80%] h-0.5 bg-gradient-to-r from-primary to-transparent" />}
            </div>)}
        </div>
      </div>
    </section>;
